...
export default class VideoLoader {
    ...
    async #init() {
        if (!this.#innertube) {
            ...
            this.#innertubeTVClient = {
                ...
                userAgent:
                    'Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/75.0.3770.142 Safari/537.36; SMART-TV; Tizen 4.0,gzip(gfe)',
            }
        }
    }
    ...
    async getInfo(video: Video): Promise<VideoInfo | null> {
        ...
        const payload = {
            ...
            playbackContext: {
                contentPlaybackContext: {
                    signatureTimestamp:
                        this.#innertube.session.player?.sts || 0,
                },
            },
        } as any
        ...
        if (video.context?.ctt) {
            ...
            delete (this.#innertube.session.context.user as any)
                ?.credentialTransferTokens
        }
        ...
        const nextResponse = (await this.#innertube.actions.execute(
            '/next',
            payload,
        )) as any
        ...
        const singleColumnContents =
            nextResponse.data?.contents?.singleColumnWatchNextResults?.results
                ?.results?.contents?.[0]?.itemSectionRenderer?.contents?.[0]
        ...
        if (videoMetadata) {
            basicInfo = {
                src: 'yt',
                title: new InnertubeLib.Misc.Text(
                    videoMetadata.title,
                ).toString(),
                channel: new InnertubeLib.Misc.Text(
                    videoMetadata.owner?.videoOwnerRenderer?.title,
                ).toString(),
            }
            ...
            basicInfo = {
                ...
                artist: new InnertubeLib.Misc.Text(
                    songMetadata.byline,
                ).toString(),
                album: new InnertubeLib.Misc.Text(
        ...
        if (basicInfo) {
            ...
            const playerResponse = (await this.#innertube.actions.execute(
                '/player',
                payload,
            )) as any

            // Wrap it in innertube VideoInfo. Why? Because it offers many useful methods like `chooseFormat()`.
            const innertubeVideoInfo = new InnertubeLib.YT.VideoInfo(
                [playerResponse],
            ...
            if (innertubeVideoInfo.basic_info.is_live) {
                ...
                const hasAudioFormat = allFormats.some((f) => f.has_audio)
                    ? 'audio'
                    : false
                const hasVideoFormat = allFormats.some((f) => f.has_video)
                    ? 'video'
                    : false
                const hasAudioAndVideoFormat = allFormats.some(
                    (f) => f.has_audio && f.has_video,
                )
                    ? 'video+audio'
                    : false
                const formatType =
                    hasAudioAndVideoFormat ||
                    hasVideoFormat ||
                    hasAudioFormat ||
                    undefined
                try {
                    const format = innertubeVideoInfo?.chooseFormat({
                        type: formatType,
                        quality: 'best',
                    })
                    streamUrl = format
